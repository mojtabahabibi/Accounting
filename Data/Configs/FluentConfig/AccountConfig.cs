...
namespace EcoBar.Accounting.Data.Configs.FluentConfig
{
    public class AccountConfig : IEntityTypeConfiguration<Account>
    {
        public void Configure(EntityTypeBuilder<Account> builder)
        {
            ...
            builder.HasOne(i => i.AccountUser).WithMany(i => i.Accounts).HasForeignKey(i => i.AccountUserId).OnDelete(DeleteBehavior.NoAction);
            builder.HasOne(i => i.AccountType).WithMany(i=>i.Accounts).HasForeignKey(i => i.AccountTypeId).OnDelete(DeleteBehavior.NoAction);
            builder.HasOne(i => i.Wallet).WithOne(i => i.Account).HasForeignKey<Account>(i => i.WalletId).OnDelete(DeleteBehavior.NoAction);
        }
    }
}